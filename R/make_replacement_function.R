...
#' The functionality automates recalling of the function to avoid collision with existing
#' replacements, and can toggle between sharing encodings between repeated keys or giving each
#' key a unique value (via the `all` parameter).
#'
#' @param encoder A zero-argument function with signature `function()` that returns
#'   a single random replacement string each call.
#' @param max_values The maximum number of unique replacements your encoder can produce.
#' @param all Logical. If `TRUE` every key gets its own unique replacement.
#'   If `FALSE` repeated keys receive the same replacement.
#' @param elevate_warnings Logical. If `TRUE`, warnings are raised as errors.
#'
