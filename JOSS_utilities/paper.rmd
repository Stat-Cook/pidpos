...
The `pidpos` package is designed to aid with the identification of personal identifiability risks in data sets.
...
a rarity for the given variable.  The `pidpos` (Personal Identifiability Detection by Part Of 
...
`pidpos` builds on the existing `udpipe` framework[@straka2016udpipe; @udpipecran], extracting all examples of 
...
approach taken in `pidpos` conversely takes the approach of purposefully extracting all proper-nouns, and hence
...
To assist with understanding the `pidpos` package, we include a subset of the 'friends' 
...
available, notably tools to automatically encode the `To` column (see [Auto Replacements](https://stat-cook.github.io/pidpos/articles/AutoReplacement.html)).
