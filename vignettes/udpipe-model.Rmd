...
The part of sentence (POS) tagging performed in `pidpos` leverage's the pretrained `udpipe` 
model framework.  This model is not included in `pidpos` due to its size, and hence has 
...
When first installed and loaded `pidpos` will not have the udpipe model.  The first time 
`pidpos()` is run, an attempt is made to download the model. Hence, it is recommended
to start by evaluating the `pidpos()` function with the example data (`the_one_in_massapequa`):
...
Assuming `pidpos()` throws an error, the next step is to check if the model is present.
...
The former of these is the more reliable, and will allow `pidpos` to operate 
...
If further problems persist please raise an issue on the `pidpos` github page.  We will
