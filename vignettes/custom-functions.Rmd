---
title: "Using custom functions in `pidpos`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using custom functions in `pidpos`}
...
Out of the box, `pidpos()` relies on the `udpipe` package to perform part-of-speech (POS) tagging.  
...
To use this with `pidpos()`, we wrap it with `custom_tagger()`:
...
We can now supply it to `pidpos()`:
...
We can bridge this into R using `reticulate` (see [this vignette](https://rstudio.github.io/reticulate/articles/calling_python.html) for an intro to using `reticulate`).
...
The preceding examples demonstrate custom taggers and filters. Once PID detection is complete, users may also wish to supply a custom replacement function for the redaction step.

Replacements are applied via `auto_replace()`, which accepts a user-defined replacement function. The examples below use `raw_redaction_rules`, a small demonstration dataset bundled with the package:
...
Users requiring more control can leverage the memoization framework built into `pidpos`. This requires first defining a zero-argument function that generates a candidate replacement string:
...
```
*Note*: letters contains 26 elements, so this encoder can produce at most 26 distinct replacements.

This function can then be passed to `make_replacement_function()`, which returns a stateful replacement function — the same pattern used internally by `make_random_replacement()`:
...
`pidpos()` can operate with virtually any POS tagging system.
