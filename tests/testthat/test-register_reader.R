test_that("register_reader validates arguments and adds function", {
  ...
  original_readers <- pidpos_env$file_readers
  pidpos_env$file_readers <- list()
  on.exit(pidpos_env$file_readers <- original_readers, add = TRUE)
  ...
  pidpos_env$file_readers
  expect_true("csv" %in% names(pidpos_env$file_readers))
  expect_identical(pidpos_env$file_readers$csv, fun)
})
